...
/*
    Mascota Saltana (Virtual Pet System)
    Sistema de mascota virtual con estadísticas, decoración de casa, minijuegos y visitas
*/

// Pet growth stages
export const petStageEnum = pgEnum('pet_stage', ['egg', 'baby', 'child', 'teen', 'adult']);

// Pet item types
export const petItemTypeEnum = pgEnum('pet_item_type', ['food', 'decoration', 'clothing', 'accessory', 'toy']);
...
export const PetsTable = pgTable('pets', {
    ...
    appearance: jsonb('appearance').notNull().default({
        ...
        accessories: [],
        clothing: []
    }), // JSON: { color, shape, accessories, clothing }
    stats: jsonb('stats').notNull().default({
        hunger: 100,
        happiness: 100,
        energy: 100,
        hygiene: 100
    }), // JSON: { hunger, happiness, energy, hygiene }
    stage: petStageEnum('stage').notNull().default('egg'),
...
export const PetHousesTable = pgTable('pet_houses', {
    ...
    layout: jsonb('layout').notNull().default({
        ...
        flooring: 'default',
        theme: 'default'
    }), // JSON: { wallpaper, flooring, theme }
    items: jsonb('items').notNull().default([]), // JSON array: [{ itemId, x, y, rotation }]
    createdAt: timestamp('created_at').notNull().default(sql`current_timestamp`),
    updatedAt: timestamp('updated_at').notNull().default(sql`current_timestamp`),
});

// Pet items catalog (shop)
export const PetItemsTable = pgTable('pet_items', {
    id: serial('id').primaryKey(),
    name: varchar('name', { length: 100 }).notNull(),
    description: text('description'),
    type: petItemTypeEnum('type').notNull(),
    price: integer('price').notNull(), // Price in Saltocoins
    icon: varchar('icon'), // URL or path to item icon
    metadata: jsonb('metadata').default({}), // Additional data: { hungerRestore, happinessBoost, etc }
    isAvailable: boolean('is_available').notNull().default(true),
    createdAt: timestamp('created_at').notNull().default(sql`current_timestamp`),
    updatedAt: timestamp('updated_at').notNull().default(sql`current_timestamp`),
});

// User's pet inventory
export const PetInventoryTable = pgTable('pet_inventory', {
    id: serial('id').primaryKey(),
    userId: integer('user_id').notNull().references(() => UsersTable.id, { onDelete: 'cascade' }),
    itemId: integer('item_id').notNull().references(() => PetItemsTable.id, { onDelete: 'cascade' }),
    quantity: integer('quantity').notNull().default(1),
    purchasedAt: timestamp('purchased_at').notNull().default(sql`current_timestamp`),
    createdAt: timestamp('created_at').notNull().default(sql`current_timestamp`),
}, (t) => ({
    uniqueUserItem: unique('user_item_idx').on(t.userId, t.itemId),
}));

// Pet visits (social feature)
export const PetVisitsTable = pgTable('pet_visits', {
    id: serial('id').primaryKey(),
    visitorId: integer('visitor_id').notNull().references(() => UsersTable.id, { onDelete: 'cascade' }),
    ownerId: integer('owner_id').notNull().references(() => UsersTable.id, { onDelete: 'cascade' }),
    likeGiven: boolean('like_given').notNull().default(false),
    giftItemId: integer('gift_item_id').references(() => PetItemsTable.id),
    createdAt: timestamp('created_at').notNull().default(sql`current_timestamp`),
});

// Pet minigame sessions
export const PetMiniGameSessionsTable = pgTable('pet_mini_game_sessions', {
    id: serial('id').primaryKey(),
    userId: integer('user_id').notNull().references(() => UsersTable.id, { onDelete: 'cascade' }),
...
// Daily limits for minigames
export const PetMiniGameLimitsTable = pgTable('pet_mini_game_limits', {
    id: serial('id').primaryKey(),
    userId: integer('user_id').notNull().references(() => UsersTable.id, { onDelete: 'cascade' }),
    gameName: varchar('game_name', { length: 50 }).notNull(),
    playsToday: integer('plays_today').notNull().default(0),
    lastPlayDate: timestamp('last_play_date').notNull().default(sql`current_date`),
    createdAt: timestamp('created_at').notNull().default(sql`current_timestamp`),
    updatedAt: timestamp('updated_at').notNull().default(sql`current_timestamp`),
}, (t) => ({
    uniqueUserGame: unique('user_game_limit_idx').on(t.userId, t.gameName),
}));
...
export const petInventoryRelations = relations(PetInventoryTable, ({ one }) => ({
    user: one(UsersTable, {
        fields: [PetInventoryTable.userId],
        references: [UsersTable.id],
    }),
    item: one(PetItemsTable, {
        fields: [PetInventoryTable.itemId],
        references: [PetItemsTable.id],
    }),
}));

export const petVisitsRelations = relations(PetVisitsTable, ({ one }) => ({
    visitor: one(UsersTable, {
        fields: [PetVisitsTable.visitorId],
        references: [UsersTable.id],
    }),
    owner: one(UsersTable, {
        fields: [PetVisitsTable.ownerId],
        references: [UsersTable.id],
    }),
    giftItem: one(PetItemsTable, {
        fields: [PetVisitsTable.giftItemId],
        references: [PetItemsTable.id],
    }),
}));

export const petMiniGameSessionsRelations = relations(PetMiniGameSessionsTable, ({ one }) => ({
    user: one(UsersTable, {
        fields: [PetMiniGameSessionsTable.userId],
        references: [UsersTable.id],
    }),
}));

export const petMiniGameLimitsRelations = relations(PetMiniGameLimitsTable, ({ one }) => ({
    user: one(UsersTable, {
        fields: [PetMiniGameLimitsTable.userId],
        references: [UsersTable.id],
    }),
}));