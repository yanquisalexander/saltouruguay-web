...
const DiscordPresenceCount = () => {
  ...
  useEffect(() => {
    const fetchDiscordPresence = async () => {
      try {
        ...
        const { presence_count } = await response.json();
        setPresenceCount(presence_count || 0);
      } catch (error) {
        // Silently handle errors (e.g., 429 rate limits)
      }
    };
  ...
  const discordUrl = SOCIAL_NETWORKS.find((network) => network.name === "Discord")?.url;

  return (
    <div class="items-center gap-2 group hidden md:flex">
      <a
        class="items-center gap-2 group hidden md:flex"
        title={`Usuarios en línea: ${presenceCount}`}
        href={discordUrl}
        target="_blank"
        rel="noopener noreferrer"
      >
        <div class="flex items-center gap-1 relative group-hover:scale-110 transform transition">
          <DiscordIcon class="size-6 text-white" />
...
export default DiscordPresenceCount;
